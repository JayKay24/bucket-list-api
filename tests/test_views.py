...
class ViewsTests(unittest.TestCase):
    ...
    def get_a_bucketlist(self, id):
        url = url_for('api.bucketlistresource', id=id, _external=True)
    ...
    def get_a_list_of_bucketlist_items(self, bkt_id):
        url = url_for('api.bucketlistitemlistresource',
                      id=bkt_id, _external=True)
        response = self.test_client.get(
            url,
    ...
    def test_get_a_single_bucketlist(self):
        """
        An authorized user should be able to retrieve a single bucketlist.
        """
        bucketlist_name = "Extreme heights"
        response = self.create_bucketlist(bucketlist_name)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_get_a_list_of_bucketlist_items(self):
        """
        An authorized user should be able to get a list of bucketlist items.
        """
        bucketlist_name = "Extreme heights"
        response = self.create_bucketlist(bucketlist_name)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        bucketlist_item_1 = "Mount Everest"
        bucketlist_item_2 = "Mount Kenya"
    ...
    def test_get_a_list_of_users(self):
        """
    ...
    def test_get_a_bucketlist_item(self):
        """
        Only an authorized user should be able to get a bucketlist item.
        """
        response_1 = self.create_bucketlist("Extreme heights")
        self.assertEqual(response_1.status_code, status.HTTP_201_CREATED)
        response_2 = self.create_bucketlist_item(1, "Mount Everest")
        self.assertEqual(response_2.status_code, status.HTTP_201_CREATED)
        response_3 = self.get_a_bucketlist_item(1, 1)
        self.assertEqual(response_3.status_code, status.HTTP_200_OK)
